...
function initCarousels() {
  ...
  const scrollToCard = (dir) => {
    ...
    const ls = lefts();
    const widths = cs.map((el) => el.offsetWidth);
...
function setupLightbox() {
  ...
  function showAt(idx) {
    ...
    const codeEl = card.querySelector('.text-sm.text-gray-500');
    imgEl.src = img?.getAttribute('src') || '';
    imgEl.alt = img?.getAttribute('alt') || '';
    capEl.textContent = codeEl ? codeEl.textContent.trim() : (imgEl.alt || '');
  // Actualizar CTA de WhatsApp con el código si existe
  const codeText = codeEl ? codeEl.textContent.replace(/^[^:]*:\s*/, '').trim() : '';
  const msg = encodeURIComponent(`Hola, me interesa el producto ${codeText || imgEl.alt}. ¿Podrían brindarme más información?`);
...
// Copiar "Código: xxx" al portapapeles con un pequeño toast
function setupCodeCopy() {
  const productos = document.getElementById('productos');
  if (!productos) return;

  // Cursor en elementos de código
  productos.querySelectorAll('.text-sm.text-gray-500').forEach((el) => {
    el.style.cursor = 'copy';
    el.title = 'Copiar código';
  });

  productos.addEventListener('click', async (e) => {
    const codeEl = e.target.closest('.text-sm.text-gray-500');
    if (!codeEl) return;
    const text = codeEl.textContent || '';
    const match = text.match(/Código\s*:\s*(.+)$/i);
...
function showToast(message) {
  ...
  if (!toast) {
    ...
    toast.id = 'mini-toast';
    toast.className = 'fixed bottom-6 left-1/2 -translate-x-1/2 bg-gray-900 text-white px-4 py-2 rounded shadow text-sm z-50';
...
function setupCategoryLoader() {
  ...
  container.querySelectorAll('.carousel.track').forEach((el) => el.classList.add('as-grid'));
...
function setupFilter(container) {
  ...
  const run = () => {
    ...
    container.querySelectorAll('.carousel.track > div').forEach((card) => {
      ...
      const code = card.querySelector('.text-sm.text-gray-500')?.textContent?.toLowerCase() || '';
      const alt = card.querySelector('img')?.getAttribute('alt')?.toLowerCase() || '';
      const show = !q || title.includes(q) || code.includes(q) || alt.includes(q);
      card.style.display = show ? '' : 'none';
    });
  };
  input.removeEventListener('input', input._filterHandler || (() => {}));
  input._filterHandler = run;
